...
An index of the eight FARP levels, each with a page of its own at `/gamemodes/levels/<n>`. That page has three views, toggled from a row of buttons under the heading:
...
The headline number on these levels is the **capture success rate** — the share of every evader that reached a verdict that the line destroyed (one still in flight when the clock stopped counts for neither side) — and the entry also reports how many reached the planet and, where attrition applies, how many defenders it cost. Each entry is graded over 100 trials, each with its own seeded defender scatter and spawn bearings, followed by a defender sweep. The detail page adds a **risk per trial** chart over the run data and a **risk vs ring size** chart (`Risk = 1 - capture success rate`) over the sweep, plus ring-sweep charts that give **every ring size its own line**, colored along a ramp from blue at the smallest ring to red at the largest — cumulative capture success rate and risk, each read wave by wave against the number of evaders faced. On the attrition levels there are three more: the same wave-by-wave reading of how many defenders were still standing, counted rather than shown as a share, how the risk moved as the submitted layout thinned, and that curve bucketed by ring size. Level 3 spends nothing, so those three would be flat by definition and are left off it. Only the first trials keep a frame recording, since these runs are far longer than a single approach; the rest are graded but not replayable.

**Levels 6, 7 and 8** are *piloted runs*, not benchmarks. In Level 6 the player flies the evader themselves against the best submitted Level 2 algorithm — and that entry's defender placements — capped at three minutes; Level 7 is the swarm merge. Any run is submitted, caught or clean. The recorded flight is uploaded and rendered by a worker into a replay, so the detail page shows the run's outcome, its detected / captured / reached-planet times, the recorded flight, and the opponent's algorithm. Charts and the ring sweep are hidden for these levels, since a single flight has no rates to plot.

**Level 8** is a piloted run of a different shape: an *allocation* problem across two planets on two separate seeds. Ten defenders start scattered around planet A and planet B starts empty, and each planet faces five evaders off the arena edges. During a two-and-a-half-minute deployment window the player flies a shuttle that carries nothing: every defender runs the submitted algorithm during the window as well as in the fight, and the shuttle reads to their sensors as one of their own, so the algorithm's own `WHEN SEES ALLY` rule is what the player steers them with. Taking the shuttle dark drops it off their sensors entirely. Crossing a defender between the planets is a summon rather than a ferry — the player drops a marker inside the placement band and one defender leaves the other planet, flies in through the jump gate and settles on it. When the window closes, planet A's assault plays out and then planet B's, both on the split the player chose and both running the one algorithm they wrote. The player flies the shuttle through both fights as well, so a recorded Level 8 frame carries sixteen slots: ten defenders, five evaders and the shuttle. The recording holds the two fights back to back over a fixed fifteen-slot layout, so it renders as a single replay. Unlike Levels 6 and 7 the headline is not a win or a loss but the **share of evaders stopped across the pair**, taken from the run's own stats, and the entry also reports the split, the breaches per planet and the crossings flown. Level 8 is scored as a defense, so its rate reads as detection rather than evasion.
...
Benchmarks run on separate **worker** processes rather than in the web server, so compute can be scaled across machines. An evaluation is **one unit of work**: the server queues it whole, a single worker claims it, and that worker runs every match in it. A Level 6, 7 or 8 piloted run is a **render job**: the run was already simulated in the game client, so the worker just packs the recorded trajectory into a replay without simulating anything.
...
| `GET` | `/api/evaluations/<id>/chart/<kind>.png` | Rendered chart PNG (`line`, `bar`, `sweep` detection rate, `capture` capture rate, `risk`, `trial-risk`, `attrition`, `sweep-attrition`, `ring-capture`, `ring-risk`, `ring-attrition`, `times`) |
