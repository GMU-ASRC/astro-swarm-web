...
<div class="max-w-4xl">
	...
	<div class="flex items-center justify-between gap-4 mt-3 mb-6">
		<h1 class="font-game text-2xl text-star-white">Simulator Run</h1>
		{#if entry}
	...
	{:else}
		{#if entry.video_filename}
			<video controls class="w-full mb-4 border-2 border-sky-500/20 bg-black">
				<source src={apiUrl(`/api/runs/${data.id}/video`)} type="video/mp4" />
			</video>
		{/if}
